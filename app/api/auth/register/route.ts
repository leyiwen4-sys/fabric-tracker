...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingResult = await execute('SELECT id FROM users WHERE email = ?', [email])
    ...
    const result = await execute('INSERT INTO users (email, password_hash) VALUES (?, ?)', [email, passwordHash])
